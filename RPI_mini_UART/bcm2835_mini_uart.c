...
static int bcm2835_uart_probe(struct platform_device *pdev)
{
    ...
    pr_info("bcm2835_uart_probe: start probe");
    pr_info("bcm2835_uart_probe: pdev-resource=%p",pdev->resource);

    //__raw_writel(1,(uint32_t *)__io_address(AUX_BASEADDR + AUX_ENABLES));

    //pr_info("bcm2835_uart_probe: pdev-resource->start=%p pdev-resource->end=%p",pdev->resource->start,pdev->resource->start);
    //pr_info("bcm2835_uart_probe: pdev-resource->name=%s",pdev->resource->name);
    //pdev-resource->start=%p pdev-resource->end=%p",pdev->resource->start,pdev->resource->start);

    base = devm_ioremap_resource(&pdev->dev, pdev->resource);

    if (IS_ERR(base)){
        pr_err("base is error\n");
        return -EINVAL;
    }
    pr_info("dafna: ok\n");
    //pr_info(KBUILD_MODNAME ":pdev->resource = %p\n",pdev->resource);
    //pr_info(KBUILD_MODNAME ":pdev->resource = %p\n",pdev->resource);


    //__raw_writel(1,(uint32_t *)base+ AUX_ENABLES);
    if (pdev->dev.of_node)
    ...
    pr_info("bcm2835_uart_probe: end probe");
...
{
    int ret;
    pr_info("bcm2835_uart_init: start init");
    ...
    pr_info("bcm2835_uart_init: end init");
